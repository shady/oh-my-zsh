...
export __GIT_PROMPT_DIR=~/.oh-my-zsh/custom/specialgit
# Initialize colors.
autoload -U colors
colors
...
setopt PROMPT_SUBST
PROMPT='%(!.$(c)(%{$fg[red]%}R%{$reset_color%}$(c)).)$(proxied)$(c)(%{$reset_color%}%n$(c)@%{$reset_color%}%m$(c)):(%{$reset_color%}%~$(c))$(gitprompt)
%b❯%{$reset_color%} '

ZSH_THEME_GIT_PROMPT_PREFIX=":($(c)"
ZSH_THEME_GIT_PROMPT_SEPARATOR="$(c)|%{$reset_color%}"
ZSH_THEME_GIT_PROMPT_SUFFIX="$(c))%{$reset_color%}"
ZSH_THEME_GIT_PROMPT_BRANCH="%{$fg_bold[magenta]%}"
